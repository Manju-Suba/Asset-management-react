...
const AuditRejectedAssets = () => {
	...
	const AssetAudit = useSelector((state) => state.AssetAuditData && state.AssetAuditData.AssetAuditByRejected);
	const AssetAuditcount = useSelector(
		(state) =>
			state.AssetAuditData && state.AssetAuditData.AssetAuditByRejected && state.AssetAuditData.AssetAuditByRejected.auditStatusCounts
	);
	...
	const handleSearchChange = (event) => {
		...
		setPageNo(1);
		getAssetAuditByStatus(1, pageSize, search.current, searchValue.current, Approvestatus, selectedCategory);
	};
	...
	const getpagerecord = (pageNumber, pageSize) => {
		...
		getAssetAuditClassData(pageNumber, pageSize);
	};
	React.useEffect(() => {
		if (!renderAfterCalled.current) {
			dispatch(getAllAssetCategoryDetails());
			getAssetAuditClassData(pageNo, pageSize);
		}
		return () => {
...
export default AuditRejectedAssets;
