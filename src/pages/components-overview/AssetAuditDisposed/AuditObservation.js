...
import { getObservation } from 'components/redux/AssetAudit/action';
...
const AuditObservation = () => {
	...
	const AssetAudit = useSelector((state) => state.AssetAuditData && state.AssetAuditData.ObservationAsset);
	const AssetAuditcount = useSelector(
		(state) => state.AssetAuditData && state.AssetAuditData.ObservationAsset && state.AssetAuditData.ObservationAsset.observationCount
	);
	...
	const Approvestatus = 'Observation';
	...
	const getAssetAuditClassData = async (pagenumber, pSize) => {
		...
		try {
			...
			await dispatch(getObservation(paagereduce, pSize, search.current, searchValue.current, selectedCategory, signal));
		} catch (error) {
			setLoading(false);
	...
	const handleSearchChange = (event) => {
		...
		setPageNo(1);
		getObservation(1, pageSize, search.current, searchValue.current, selectedCategory);
	};
	...
	const handleCategoryChange = async (value) => {
		...
		try {
			const controller = new AbortController();
			const signal = controller.signal;
			await dispatch(getObservation(1, pageSize, search.current, searchValue.current, value, signal));
		} catch (error) {
			setLoading(false);
			console.error('Error fetching data:', error);
		} finally {
	...
	const columns = [
		...
		// 	render: (_, record) => (
		// 		<Space size="middle">
		// 			<img src={API_BASE_URL + record?.currentImageWithPath || ''} alt={'Current'} style={{ width: '30px' }} />
		// 		</Space>
		// 	)
		// },
		{
			title: <span className="table-hd-fs">Observation Remarks</span>,
	...
	const aList = AssetAudit?.observationData || [];
