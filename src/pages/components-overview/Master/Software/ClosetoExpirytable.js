...
import { getCloseToExpiry } from 'components/redux/software/action';
...
const ClosetoExpirytable = () => {
  const dispatch = useDispatch();

  const SoftwareClose = useSelector((state) => state.SoftwareData && state.SoftwareData.CloseToExpiry);
  const [selectedCategory, setSelectedCategory] = useState('Choose Asset Category');
  const [selectedAssetType, setSelectedAssetType] = useState(null);

  // const SoftwareClose = useSelector((state) => state.AssetData && state.AssetData.SoftwareClose);
  ...
  const handleCategoryChange = (value) => {
    ...
    dispatch(getCloseToExpiry(value, selectedAssetType));
    // setAssetTypes(AssetTypeDetails || []);
  };

  const handleAssetTypeChange = (value) => {
    setSelectedAssetType(value);
    dispatch(getCloseToExpiry(selectedCategory, value));
  };
  ...
  React.useEffect(() => {
    ...
    dispatch(getCloseToExpiry(selectedCategory, selectedAssetType));
  }, [dispatch]);
  ...
  return (
    <>
      ...
      <Table className="table-hd" columns={columns} dataSource={SoftwareClose} />
    </>
  );
};

export default ClosetoExpirytable;
