...
const UserTransferRejectedTable = () => {
	...
	const Transferstatus = 'Rejected';
	const AssetList = useSelector((state) => state.AssetTransferData && state.AssetTransferData.AssetTransferReject);
	...
	const getAssetAllocationData = async (pagenumber, Size) => {
		...
		try {
			...
			await dispatch(
				getAssetTransferByStatus_Users(Transferstatus, selectedCategory, null, selectedAssetType, paagereduce, Size, signal)
			);
		} catch (error) {
			setLoading(false);
			console.error('Error fetching data:', error);
...
export default UserTransferRejectedTable;
