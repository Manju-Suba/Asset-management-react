...
// image
import { getPendingAssetTicket, getAllAssetTicketNo, sapActionUpdate } from 'components/redux/AssetCreation/action';
...
const AllassetTicket = () => {
	...
	const ticketType = useRef('Pending');
	...
	const handleApproveAction = (tickerId) => {
		try {
			dispatch(sapActionUpdate(tickerId, 'Approved'))
				.then((response) => {
				...
				<Space>
					<Grid item sx={{ color: '#A5A1A1', fontSize: '10px' }}>
						<div
							...
							tabIndex="0"
							onClick={() => handleApproveAction(record.ticketNo)}
							onKeyDown={(e) => {
								if (e.key === 'Enter') {
									handleApproveAction(record.ticketNo);
								}
							}}
						>
	...
	React.useEffect(() => {
		dispatch(getAllAssetTicketNo('Pending'));
