...
import { getStatusWiseAsset } from 'components/redux/DisposalAsset/action';
...
const ReplacedAsset = () => {
  ...
  const StatusWiseAsset = useSelector((state) => state.DisposalAssetData && state.DisposalAssetData.StatusWiseAsset);
  const AssetListcount = useSelector(
    (state) => state.DisposalAssetData && state.DisposalAssetData.StatusWiseAsset && state.DisposalAssetData.StatusWiseAsset.replacedCount
  );
  ...
  const getAssetAllocationData = async (pagenumber, Size) => {
    ...
    try {
      ...
      await dispatch(getStatusWiseAsset(status, selectedCategory, paagereduce, Size, search.current, searchValue.current, signal));
    } catch (error) {
      setLoading(false);
  ...
  const assetList = StatusWiseAsset?.replacedassets || [];
  ...
  const handleCategoryChange = async (value) => {
    ...
    try {
      ...
      await dispatch(getStatusWiseAsset(status, value, 0, pageSize, search.current, searchValue.current, signal));
    } catch (error) {
      setLoading(false);
      console.error('Error fetching data:', error);
    } finally {
  ...
  const columns = [
    ...
    },
    {
      ...
      render() {
        return <span className="repalced-table-value-badge">Replaced</span>;
      }
    },
    {
      dataIndex: 'plant',
  ...
  return (
    <>
      ...
      <div className="align-center-data">
        <Pagination defaultCurrent={pageNo} current ={pageNo} total={AssetListcount} pageSize={pageSize} onChange={getpagerecord} hideOnSinglePage={true} />
      </div>
    </>
  );
};
export default ReplacedAsset;
