...
import { getAllAssetRetrialDetails } from 'components/redux/asset/action';
...
const Retrailtable = ({ onRowClick }) => {
	const dispatch = useDispatch();
	const [selectedAsset, setSelectedAsset] = useState(null);
	const AssetAllocated = useSelector((state) => state.AssetData && state.AssetData.AssetRetrail);
	...
	const handleCategoryChange = (value) => {
		...
		dispatch(getAllAssetRetrialDetails(value, selectedAssetType));
	};

	const handleAssetTypeChange = (value) => {
		setSelectedAssetType(value);
		dispatch(getAllAssetRetrialDetails(selectedCategory, value));
	};
	const columns = [
		{
	...
	React.useEffect(() => {
		...
		dispatch(getAllAssetRetrialDetails(selectedCategory, selectedAssetType));
	}, [dispatch]);
	...
	return (
		<>
			...
			{selectedAsset ? (
				<ViewAsset onRowClick={handleRowClick} />
			) : (
				<Table
					className="table-hd"
					columns={columns}
					dataSource={AssetAllocated}
