...
const TransferApprovedTable = ({ onRowClick }) => {
  ...
  const AssetTransfer = useSelector((state) => state.AssetTransferData && state.AssetTransferData.AssetTransferApprove);
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'status',
      render: () => {
        return <span className="table-value-badge-success">{'Approved'}</span>;
      },
      sorter: {
...
export default TransferApprovedTable;
